...
// S6a — 주간 회고: 이번 주를 비추는 '읽기용 거울'. 결정 의무 제거(탈압박) — 놓아줄지 같은 결정은
// 저녁 결정 드립(EveningPrompt)이 하나씩 묻고, 즉흥 결정·합치기는 주제 탭에서. 여기선 증거·흐름·서술만.
    ...
    var body: some View {
        List {
            ...
            Section {
                ...
                Text("지속 후보 — 이번 주 돌아온 주제")
            } footer: {
                Text("↑ 떠오름·↓ 식어감은 최근 흐름, 🌱 진화·🔁 맴돎은 생각이 옮겨갔는지예요. 이건 이번 주를 비추는 거울 — 놓아줄지 같은 결정은 저녁에 하나씩 찾아와요(또는 주제 탭에서).")
            }
            ...
            if !cooling.isEmpty {
                Section {
                    ...
                    Text("한때 자주 다뤘는데 요즘 조용해진 주제예요. 놓아줄지는 저녁 알림이 하나씩 물어봐요. (지속 = 이어가기 + 가지치기)")
                }
            }
            ...
            if !connections.isEmpty {
                Section {
                    ...
                    Text("서로 가까운 주제예요. '새 연결'은 이번 주 처음 가까워진 것. 한 줄기면 주제 탭에서 묶을 수 있어요. (추정 — 참고용)")
                }
            }
