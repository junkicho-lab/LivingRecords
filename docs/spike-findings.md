...
## ⑨ (post-v1) 연결 'now' 벡터: 전체누적 vs 창한정 — 차이 없음, 변경 보류 (2026-06-21)
- 의심: `WeeklyReview.connections`의 `now`=주제 *전체* 포착 평균이라 "이번 주 새로 가까워진"(isNew) 의도와 어긋날 수 있음(최근 수렴이 옛 포착에 희석).
- 스파이크(`connection_window_spike.swift`): 합성 4주제로 `now全`(전체누적) vs `now窓`(창한정)의 link/isNew 판정 비교 → **6쌍 전부 분류 동일.** 창 한정으로 바꿔도 결과 안 바뀜.
- 원인: ⑥·⑧의 임베딩 거칢이 지배적 — 유사도가 0.13~0.55로 몰려 임계값(0.15) 부근에서 `now` 산식 차이가 분류를 못 뒤집음. 무관한 '직장~식습관'도 prior 0.251로 안 갈림.
- **판정: `now`를 창 한정으로 바꾸는 건 근거 없음 → 변경 보류.** 연결을 정교화하려면 `now` 산식이 아니라 임베딩 층(top-K 후보·재랭킹, ⑥ 향후)을 손대야 한다.
- ⚠️ 합성 4주제 기준의 제한적 결과. 실데이터에서 재확인 여지.
