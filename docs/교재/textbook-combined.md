...
- **Review questions:** Why was UI feedback during recording needed (several UX commits after `12f89de`)?
...
- **Commits:** `e66c50f`, `96b488e` (sealed separation), `347848e` (theme `[[links]]` and re-mirroring).
...
- **Commits:** `0baf42d`, `24194a3`, `8f6aea5`.
...
- **Commits:** `cf7c950` (S6a), `4ade900` (S6b).
...
- **Commits:** `6378825`·`d4cde74` (CER measurement), `6165227` (filler words), `6344aba`~`35ec46a` (tab swipe), `38bccef`~`087793c` (the struggle to auto-expand the input box).
- **Takeaway:** Fixing the single "auto-expanding input box" took four commits (`38bccef` → `087793c`). *Even a small bit of UX breaks and gets fixed several times on a real device.* A passing build ≠ working.
...
- **Commit:** `0927954`. **File:** `WeeklyReview.swift`.
...
- **Commit:** `6b1940d`. **Spike:** ⑦ (`intention_spike*.swift`).
...
- **Commits:** `a680230` (introduced), `97e8af5`.
...
- **Commit:** `0490d8e`. **Spike:** ⑧ (`search_spike.swift`).
...
- **Files:** `Backup.swift`. **Commit:** `aed31ab`. **Spike:** `backup_spike.swift` (round-trip and wrong-password-failure verification).
...
- **Files:** `LocalSynth.swift` (+ unifying 6 call sites). **Commit:** `adb6934`.
...
- **Files:** `Templates.swift`, `TemplatesView.swift`, `Models.swift` (CustomTemplate). **Commit:** `12c5c5b`.
...
- **Files:** `InsightsView.swift`, `SettingsView.swift` (category separation). **Commit:** `0692062`.
...
- **Files:** `CaptureView.swift`. **Commit:** `93d3ed3`.
...
- **Files:** `Resurfacer.swift`, `InsightsView.swift`. **Commit:** `93d3ed3`.
...
- **Files:** `Reminders.swift`, `ContentView.swift`. **Commit:** `76ef6d7`.
...
- **Files:** `WeeklyReview.swift` (Precedent). **Commit:** `c065fb5`.
...
- **Files:** `Throughline.swift`, `InsightsView.swift`. **Commit:** `7b4de04`.
...
- **Files:** `WikiBuilder.swift`, `ObsidianMirroring.swift`, update hooks (`Consolidation`/`Curation`). **Commit:** `395757a`.
...
- **Files:** `WikiSummary.swift`, `Models.swift` (Theme.summary). **Commit:** `5daed48`.
...
- **Commits:** `c8c36db` (tuning), `ab886ee` (removal).
...
- **What happened:** Two-way sync (import, delete, auto-sync) was introduced (`184c6a9`) → a **duplication incident** → withdrawn (`59f559c`).
...
- **Read:** compare the diff of commit `184c6a9` (introduced) ↔ `59f559c` (withdrawn).
...
- **Files:** `PeriodReview.swift` (`!sealed` filter), `Interfaces.swift` (stub removal). **Commit:** `232fa5a`.
...
### 8-4. All 67 Commits
Check with `git log --reverse --oneline`. From S0 (`a89972f`) → full review (`232fa5a`).
