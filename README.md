...
> 이 앱은 **S0(스캐폴딩)부터 한 슬라이스씩** 자라왔다. 아래는 큰 흐름 — 단위별 상세는 [`docs/v1-slices.md`](docs/v1-slices.md)~[`docs/v4-slices.md`](docs/v4-slices.md), 전체 해설은 [`docs/프로젝트-보고서.md`](docs/프로젝트-보고서.md). 커밋으로 따라가려면 `git log --reverse --oneline`.

| 버전 | 슬라이스 | 무엇이 들어왔나 |
|---|---|---|
| **v1 — 기반** | S0–S7 | 스캐폴딩 → 음성·텍스트 포착 → 에너지·봉인 → Obsidian 단방향 미러 → 임베딩·FM 주제 분류/큐레이션 → 일일·주간·기간 정리 → 클라우드 깊은 종합(옵트인) |
| **v2 — 지속 루프** | S8–S11 | 실행 추적(의도 감지·약속·생존 판정) · 냉각(식어가는 줄기·되살리기/접기) · 연결 심화(강도·새 연결) · 피드백('지난 회고 이후'로 루프 폐곡선) |
| **v3 — 포착 도달성** | S12–S14 | App Intent(액션버튼·Siri·AirPods) · 저녁 회고 리마인더 · 포착 위젯(홈·잠금화면·제어센터) |
| **v4 — 깊이** | S15~ | 하이브리드 검색(전문+의미) · 암호화 백업/복원 · 커스텀 정리 템플릿 · '흐름' 시각화(Charts) · 되새김(과거가 현재로) · 선례 · 긴 호흡의 줄기 · 기간 칩 · LLM 위키(주제 허브·index, 단방향 — 양방향 미러는 중복 위험으로 **철회**) |

### 현재 작업 중 (미커밋)

위젯 봉인 포착 진입점(잠금화면 가로·제어센터)에 이어, 다음을 더하는 중 — 큰 줄기는 **분류 품질**과 **사용자 편집권**:

- **음성 전사 안정화** — STT 에셋 '설치 → 예약' 순서 교정 + 재시도 (실기기 *"예약됨 0/5"* 오류 해결)
- **봉인 일관성 확장** — 봉인 포착이 섞인 일일·주간 정리도 `봉인/` 폴더로 (봉인 보장을 정리 산출물까지)
- **분류 ① 로컬 재작성** — 임베딩 top-K 후보 축소 + 주제 *내용* 기반 최적 매칭 + 이름 세분화
- **분류 ② 옵트인 클라우드 분류** — 원할 때만 원문을 Claude로 보내 분류 (깊은 종합과 *별도 동의*, 봉인 제외, 전송 로그)
- **분류 ③ 사용자 교정 학습** — 직접 옮긴 포착을 '확정 범례'로 기억해 다음 배정을 보정 (온디바이스, 새 전송 0)
- **포착 원문 수정** — 주제·기록 탭에서 음성 오탈자 교정 (임베딩·미러·허브 동시 갱신, 재분류는 안 함)
- **정리 편집** — 정리 탭에서 '내 생각' 직접 쓰기 + 기존 정리 문서 수정
